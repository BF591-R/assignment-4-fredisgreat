...
intensity <- read_data('data/example_intensity_data.csv', ' ')
head(intensity, 2)
```
### Background on PCA 
...
and the cumulative sum of variance explained. Name these columns
"principal_components", "variance_explained", and "cumulative" respectively.
Return the first five rows below.__
...
caption_4 <- 'Descriptive Caption for part 4 here'
caption_5 <- 'Descriptive Caption for part 5 here'
caption_8 <- 'Descriptive Caption for part 8 here'
```

## 4. Plot the variance explained and cumulative variance for the PCs
__In the Rmarkdown code block below, use the `variance_tibble` generated in part
3 , and create a *plot* using ggplot2 that contains both a barchart representing
the variance explained by each individual PC along with a scatterplot (connected
with a line) that displays the cumulative sum of the variance explained across
all the PCs. Provide appropriate labels for each plot. Refer to the plot found in 
the reference_report.html for inspiration.__
...
PC1, PC2, PC3, etc.). You may run into an issue where your axis is automatically
sorted differently. Please refer to discussions on lexicographical ordering vs.
numeric ordering.

```{r code block to plot variance explained and cumulative variance explained, fig.cap = caption_4}
#Create a ggplot2 with the plot described above, refer to the reference report
...
bar_cols <-c('Variance Explained'='light blue')
  variance_tibble %>%
    ...
    scale_x_discrete(limits = variance_tibble$principal_components) +
...
make_biplot('data/proj_metadata.csv', pca_results)
```

## 6. Filter the differential_expression_results.csv for significant probes
We have provided you with sample results of differential expression analysis
performed on the intensity data from part 1, contained in a CSV file. We are
going to extract a list of probes that are considered differentially expressed,
and eventually generate a heatmap of the normalized intensity values for these
selected probes.

Write a function, list_significant_probes, that works with a tibble of the
differential expression results, filters the data to only contain significant
probes with a 'padj' of less than .01, and returns a list of the significant
probe IDs. Ensure your function defines two arguments: the tibble of
differential expression results and a user-specified value for the 'padj'
threshold.
...
# read the differential expression results in as a tibble first

diff_exp_tibble <- readr::read_csv("data/differential_expression_results.csv",
                                   skip=1,
                                   col_names=c("probeid","t","p","padj")
                                   )

sig_ids <- list_significant_probes(diff_exp_tibble, .01)
head(sig_ids)
length(sig_ids)
```
